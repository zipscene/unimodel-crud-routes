...
const { ObjectMask, deepCopy, setPath, getPath, isScalar } = require('zs-objtools');
...
class ModelAccessWrapper extends CrispHooks {
	...
	query(params) {
		...
		this._preprocessParams(params, 'query');
		let model = this.model;
		return Promise.resolve()
	...
	stream(params) {
		...
		this._preprocessParams(params, 'stream');
		let model = this.model;
		return Promise.resolve()
			.then(() => this.trigger('pre-query', params))
			.then(() => {
	...
	count(params) {
		...
		this._preprocessParams(params, 'count');
		let model = this.model;
		return Promise.resolve()
	...
	aggregateMulti(params) {
		...
		this._preprocessParams(params, 'aggregateMulti');
		let model = this.model;
	...
	put(params) {
		...
		this._preprocessParams(params, 'put');
		let data;
		let model = this.model;
		let keys;
		return Promise.resolve()
			...
			.then(() => {
				...
				// Perform the update
				return model.upsert(keys, update, { allowFullReplace: true });
			})
			.then((result) => {
				if (result.data) {
	...
	upsert(params) {
		...
		this._preprocessParams(params, 'upsert');
		let model = this.model;
		return Promise.resolve()
	...
	update(params) {
		...
		this._preprocessParams(params, 'update');
		let model = this.model;
		let query, update;
		return Promise.resolve()
	...
	remove(params) {
		...
		this._preprocessParams(params, 'remove');
		let model = this.model;
		let query;
		return Promise.resolve()
