...
const { ObjectMask, deepCopy, setPath, getPath, deletePath, isScalar, merge } = require('objtools');
...
class ModelAccessWrapper extends CrispHooks {
	...
	constructor(options = {}) {
		...
		this.model.schema.traverseSchema({
			onSubschema: (subschema, path) => {
				if (subschema.private) {
					let newMask = new ObjectMask({}).addField(path);
					this.publicMask = ObjectMask.subtractMasks(this.publicMask, newMask);
					this.hasPrivateFields = true;
				}
			}
		});
		this.privateMask = ObjectMask.invertMask(this.publicMask);
		...
		this.publicSchema = this.model.schema.filterSchema((subschema, path) => {
	...
	put(params) {
		...
		return Promise.resolve()
			...
			.then(() => {
				...
				// Check permissions
				if (params.permissions) {
					// Test permissions against an update from a new doc to the passed data,
					// which allows creating docs when the user does not have permissions for defaulted fields.
					const initial = model.create();
					const changedData = Update.createFromDiff(initial.data, data).$set;
					const grant = params.permissions.getTargetGrant(this.options.permissionsTarget, changedData);
					grant.checkMask('writeMask', changedData);
				}
				...
				// Get the keys of the object to form the query to find the currrent document
				keys = {};
				for (let field of this.keyFields) {
				...
				return this.model.findOne(keys)
					.then((doc) => {
						...
						if (!params.allowPrivateFields && this.hasPrivateFields) {
							let existingPrivateFields = this.privateMask.filterObject(doc.data);
	...
	upsert(params) {
		...
		return Promise.resolve()
			...
			.then(() => {
				// Make the update
				let update = createUpdate(params.update, {
					allowFullReplace: false,
					schema: model.getSchema && model.getSchema()
				});

				// Make the query
				let query = createQuery(params.query, {
					schema: model.getSchema && model.getSchema()
				});
	...
	update(params) {
		...
		return Promise.resolve()
			...
			.then(() => {
				...
				update = createUpdate(params.update, {
					allowFullReplace: false,
					schema: model.getSchema && model.getSchema()
				});
				if (params.permissions) {
					...
					try {
						...
						grant.check('write');
						grant.check(update.getUpdatedFields(), 'writeMask.');
