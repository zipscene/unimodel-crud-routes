...
class ModelAccessWrapper extends CrispHooks {
	...
	put(params) {
		...
		return Promise.resolve()
			...
			.then(() => {
				...
				this.model.getSchema().traverseSchema({
					onSubschema(subschema, path) {
						if (subschema.protected) deletePath(data, path);
					}
				});
				...
				return this.model.findOne(keys)
					.then((doc) => {
						if (params.permissions) {
							this._checkDiffPermissions(params.permissions, doc, data);
						}
					...
					.catch((err) => {
						...
						if (params.permissions) {
							this._checkDiffPermissions(params.permissions, model.create(), data);
						}
	 ...
	 */
	_checkDiffPermissions(permissions, doc, data) {
		...
		let protectedFields = [];
		this.model.getSchema().traverse(data, {
			onField(path, value, subschema) {
				if (subschema.protected) {
					...
					if (value !== undefined) {
						throw new XError(XError.INVALID_ARGUMENT, 'Target document has protected fields.');
					}
				}
			}
		});
		fields = _.pullAll(fields, protectedFields);
