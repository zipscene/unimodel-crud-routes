...
class ModelAccessWrapper extends CrispHooks {
		...
		return Promise.resolve()
			...
			.then((data) => {
				...
				if (params.filterPermissionsFunc) {
					...
					if (!data) throw new XError(XError.ACCESS_DENIED);
				}
	...
	get(params) {
		...
		for (let field in keys) {
			...
			if (!isScalar(keys[field]) || keys[field] === null || keys[field] === undefined) {
	...
	stream(params) {
		...
		return Promise.resolve()
			...
			.then(() => model.findStream(params.query, params))
			.then((stream) => {
