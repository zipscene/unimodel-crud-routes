...
class ModelAccessWrapper extends CrispHooks {
		...
		this.model.schema.traverseSchema({
			onSubschema: (subschema, path) => {
				if (subschema.private) {
					this.privateMask.addField(path.replace(/\$/g, '_'));
					this.hasPrivateFields = true;
				}
				if (subschema.protected) {
					this.protectedMask.addField(path.replace(/\$/g, '_'));
		...
		return Promise.resolve()
			.then(() => {
				...
				return Promise.resolve()
					...
					.then(() => this.trigger('post-permissions', params, 'get'))
	 ...
	 *   @param {Boolean} params.skipQueryPermissions
	...
	query(params) {
		...
		return Promise.resolve()
			...
			.then(() => {
				if (params.permissions && !params.skipQueryPermissions) {
			...
			.then(() => this.trigger('post-permissions', params, 'query'))
	...
	stream(params) {
		...
		return Promise.resolve()
			...
			.then(() => this.trigger('post-permissions', params, 'stream'))
	...
	count(params) {
		...
		return Promise.resolve()
			...
			.then(() => this.trigger('post-permissions', params, 'count'))
	...
	aggregateMulti(params) {
		...
		return Promise.resolve()
			...
			.then(() => {
				...
				return this.trigger('post-permissions', params, 'aggregateMulti')
					.then(() => {
						// Run the aggregate
	...
	upsert(params) {
		...
		return Promise.resolve()
			...
			.then(() => {
				...
				// Perform the update
				// Perform the update
				return model.upsert(query, update);
			})
			.then(prof.wrappedEnd());
	}
