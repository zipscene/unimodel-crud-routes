...
class ModelAccessWrapper extends CrispHooks {
	...
	_checkDiffPermissions(permissions, doc, data) {
		...
		// Find defaulted fields
		schema.traverseSchema({
			onSubschema: (subschema, path) => {
				if (subschema.default !== undefined && getPath(data, path) === undefined) {
					defaultedFields.push(path);
				}
			}
		});

		// Skip permissions checks on defaulted fields
		fields = _.pullAll(fields, defaultedFields);
